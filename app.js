...
useEffect(() => {
  ...
  const handleFinalizeMessage = (finalMessageObject) => {
    ...
    // THE FIX: Check if the incoming message's conversation is the one currently open.
    // We use selectedConversationRef.current because it is guaranteed to be up-to-date.
    if (!selectedConversationRef.current || selectedConversationRef.current.id !== conversation_id) {
        playNotificationSound();
    }
